...
global rooms #전역변수로 DB처럼 저장되는 rooms 생성
rooms = [] # 생성되는 채팅방들
...
def joinChatRoom(joinUser, joinNumber): ###========================수정 필요함==============================
...
def Send(group, send_queue): #group에는 보낼 클라이언트의 소켓정보(튜플), send_queue에는 큐에담긴 데이터가 모두 들어감
    ...
    while True: #종료조건이 Ture인 무한루프
        try:
            #==================================1. 이부분 수정해야 함, 방향: room 리스트 안의 같이 소속된 클라이언트에게만 출력되도록==========
            # ===========의문: 이부분을 수정하게 되면 쓰레드 생성로직이 더 복잡해지지 않나?
            ...
            ##문자열 보내는 부분##
            for conn in group: #모든 클라이언트만큼 반복 #소켓정보(변수)를 클라이언트 채팅방 목록속 채팅방의 리스트 요소만큼 순서대로 반복 ex)[[1,a,b,c,0]]??
                message = str(recv[0])# 정수나 실수를 문자열의 형태로 변환하는 str함수에 큐의 첫번째
                                    #즉, send_queue의 첫번째 데이터를 넣어서 문자열화 시킨다.

            #==================================아니면 2.이부분을 수정해야 함, 방향: room 리스트 안의 같이 소속된 클라이언트에게만 출력되도록========
            #==================================지금형태:모든 클라이언트 터미널에 클라이언트 메세지가 전부 출력된다.================================
            #여기를 수정하게 되면 아마도 메세지는 모든 클라이언트에게 전송이 되지만 메세자를 출력되는 부분만 수정하게 된다.
                if recv[1] != conn: #만약 recv[]의 1번 인덱스(2번째 인덱스)가 conn(변수):client 본인이 아니면   
                    #client 본인이 보낸 메시지는 받을 필요가 없기 때문에 제외시킴

                    ####==========반복문 필요(같은 채팅방에 있는 클라이언트에게 순서대로 전송)==================

                    #서버가 클라이언트 창에 출력하고자 하는 메세지 클라한테 송신
                    ...
                    print(message) # 클라이언트 터미널에 출력되는 메세지를 서버창에도 출력    
                else:
                    pass #구문상 필요하지만 작업이 필요없어서 내부동작 x    
        except:
            pass #구문상 필요하지만 작업이 필요없어서 내부동작 x

#recv[0] == 문자열, recv[1] == 보낸 클라이언트의 소켓정보 , recv[2~]: 보낼 클라이언트의 소켓정보
...
try:
    while True:
        ...
        recvPhoneNumber = conn.recv(1024).decode()#클라이언트로 부터 최초의 전화번호를 입력받음
        user_id = check_user(recvPhoneNumber)#데이터베이스 관련 함수에 전화번호 추가!
        ...
        while True:
            show_menu() #메뉴판 출력
            #클라이언트로부터 메뉴번호 수신 받아야함
            role = int(conn.recv(1024).decode()) #클라이언트로 부터 메뉴 숫자를 입력받음 (문자열을 받아와서 인트값으로 변환)
            if role == '1':  # 게시글 생성
                    #클라이언트로 부터 탑승 시간을 입력받음 (HH:MM 형식)? 문자열? int? 일단 인코딩 디코딩 제외
                    ride_time_str = conn.recv(1024)
                    ## 서버가 수신받고 DB에 저장
                    ...
                    ##클라이언트가 송신
                    pickup_location = conn.recv(1024).decode()#클라이언트로 부터 탑승지를 입력받음

                    destination = conn.recv(1024).decode()#클라이언트로 부터 목적지를 입력받음

                    ## 서버가 수신받고 DB에 저장 이때 유저 id는 데이터베이스에서 새로 꺼내와야함
                    ...
                    db_connection.commit()
                    print("게시글이 생성되었습니다.") #클라이언트에게도 출력, 서버가 송신해야함
                    created_noti1 = "게시글이 생성되었습니다."
                    conn.send(bytes(created_noti1.encode())) #클라이언트에게도 공지를 출력


                    #================수정 필요==============
                    createUser = user_id #채팅방장 유저 정보  ==> DB에서 가져와야함
                    #================수정 필요==============
                    createNumber = post_id #생성된 게시판 번호 DB에서 가져와야함
                    #====================================== 
                    ...
                    for post in posts:
                        ...
                        created_noti2 = f"게시글 ID: {post_id}, 탑승 시간: {ride_time}, 탑승지: {pickup_location}, 목적지: {destination}, 소유자 ID: {owner_id}"
                        conn.send(bytes(created_noti2.encode())) #클라이언트에게도 게시글 목록 공지를 게시글 개수 만큼 송신

                    post_id_to_join = conn.recv(1024).decode()#클라이언트로 부터 참여하려는 게시글의 ID를 입력받음
                    # 게시글에 참여 #서버가 수신받고 입력
